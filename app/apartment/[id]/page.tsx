  ...
  return (
    <div className='min-h-[calc(100vh - 80px)] grid grid-cols-2 gap-4 px-12 py-8  place-items-center  max-sm:grid-cols-1'>
      ...
      <div className='font-semibold flex flex-col'>
        ...
        <p> <span className='font-normal'>depozyt:</span>  {apartment?.deposit} PLN</p>
        ...
        <p className='font-normal tracking-wide mb-4'>{apartment?.description}</p>
        <Link href={`/payment/${apartment?.id}`} className='px-4 py-1 bg-primary text-primary-foreground rounded-sm self-end '>Wynajmuje</Link>
      </div>
    </div>
  )
}
